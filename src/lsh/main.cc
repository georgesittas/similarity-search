...
int main(int argc, char* argv[]) {
  ...
  int table_size = input_points.size() / 8; // Tune-able

  LshNnSolver solver(input_points, width, table_size, config.num_tables, config.num_hashes);
  std::cout << "Done! (" << Stopwatch(STOP) / 1000.0 << " sec)\n\n";
  ...
  while (true) {
    double bf_duration, lsh_duration;
    ...
    double sum_approx_dist = 0.0, avg_lsh_duration = 0.0;

    double avg_duration_rate = 0.0; // avg { lsh duration / brute force duration }
    ...
    for (const DataPoint* query_point : query_points) {
      ...
      approx_nns = solver.ApproxKNns(query_point, config.num_neighbors, lsh_duration);
      ...
      for (int i = 0, approx_count = approx_nns.size(); i < approx_count; i++) {
        output_file << "\n\nNearest neighbor-" << i + 1 << ": " << approx_nns[i].first->GetId()
                    <<   "\ndistanceLSH: " << approx_nns[i].second
      ...
      avg_lsh_duration += lsh_duration;
      avg_duration_rate += lsh_duration / bf_duration;
      ...
      std::vector<const DataPoint*> data_points_in_range =
        solver.ApproxRangeSearch(query_point, 0, config.search_radius, 20 * config.num_tables);
    ...
    std::cout << "Done! Computed the following statistics (over all queries):\n\n"
              ...
              << "avg_lsh_duration   : " << avg_lsh_duration / query_points.size() << "ms\n"
